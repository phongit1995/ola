...
export function LegalDocument({ namespace }: { namespace: LegalNamespace }) {
  ...
  const sections = t(`${namespace}.sections`, { returnObjects: true }) as unknown as LegalSection[];
    ...
    <ScrollView className="flex-1 bg-[#f0f7f0]">
      <View className="bg-ola-primary-dark px-4 pb-6 pt-4">
        <Pressable className="self-start py-1" onPress={() => navigation.goBack()}>
          <Text className="text-sm text-white/90">{t(`${namespace}.back`)}</Text>
        </Pressable>
        <View className="mt-3 items-center">
          ...
          <Text className="mt-1.5 text-[13px] text-white/85">{t(`${namespace}.subtitle`)}</Text>
        </View>
      </View>

      <View className="px-4 pb-14 pt-6">
        <Text className="mb-4 text-center text-xs text-[#5a7a5d]">{t(`${namespace}.updated`)}</Text>
        ...
        <Text className="text-center text-xs text-[#7a9a7d]">{t(`${namespace}.footer`)}</Text>
      </View>
    </ScrollView>
  );
}
