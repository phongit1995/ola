...
function InfoRow({ icon, text, note }: { icon: number; text: React.ReactNode; note?: boolean }) {
  ...
  return (
    <View className="ml-4 mt-2 flex-row items-center gap-1">
      ...
      <Text
        className="text-xs"
        style={note ? { color: '#33691e', fontStyle: 'italic' } : { color: 'rgba(0,0,0,0.54)' }}
      >
        {text}
      </Text>
    </View>
...
export function UserProfileScreen({ username, language, onClose, onOpenProfile, onMessage, onEditProfile }: UserProfileScreenProps) {
  ...
  const reloadPosts = useCallback(async () => {
    ...
    const result = await MeService.byUser(profile.id, { limit: 30 }).catch(() => null);
  ...
  const toggleReaction = useCallback(
    async (id: string, type: 'like' | 'dislike') => {
      ...
      setPosts((prev) =>
        prev.map((p) => (p.id === id ? applyPostReaction(p, active ? null : type, selfLiker()) : p))
      );
      try {
  ...
  function buildPostMenuOptions(post: Post): ListOption[] {
    ...
    if (mine) {
      return [
        ...
        { key: 'delete', label: t('me.menuDelete'), danger: true, onSelect: () => setDeletePostId(post.id) },
      ];
    }
    return [
      {
      ...
      { key: 'save', label: t('me.menuSave'), onSelect: () => push('success', t('me.saveSuccess')) },
      { key: 'share', label: t('me.menuShare'), onSelect: () => push('success', t('me.shareSuccess')) },
      {
        key: 'block',
  ...
  const nick =
    profile?.fullName != null && profile.fullName !== '' ? profile.fullName : profile?.username ?? username;
      ...
      <View className="flex-1 bg-[#f3f3f3]">
          ...
          <ScrollView className="flex-1">
            <View className="mb-2 bg-white" style={CARD_SHADOW}>
              <View style={{ paddingBottom: 48 }}>
                <ImageBackground
                  ...
                  style={{ width: '100%', aspectRatio: 16 / 9, backgroundColor: DEFAULT_COVER_COLOR }}
                >
                  {profile.coverPhoto != null && (
                    <Pressable className="absolute inset-0" onPress={() => openViewer([profile.coverPhoto!])} />
                  )}
                  {isSelf && (
                    <Pressable
                    ...
                    >
                      ...
                      {profile.spouse != null && (
                        <Pressable onPress={() => onOpenProfile(profile.spouse!.username, colorForName(profile.spouse!.username))}>
                          <Avatar name={profile.spouse.username} uri={profile.spouse.avatar ?? undefined} size={96} rounded={false} />
                        </Pressable>
                      )}
                    </View>
                  </View>
              ...
              <View className="flex-row items-center justify-center gap-1 p-2">
                <Text numberOfLines={1} className="min-w-0 shrink text-lg" style={{ color: 'rgba(0,0,0,0.87)' }}>
                  @{profile.username}
                  {profile.fullName != null && profile.fullName !== '' && (
                    <Text style={{ color: 'rgba(0,0,0,0.54)' }}> · {profile.fullName}</Text>
                  )}
                </Text>
                {profile.verified && <Image source={checkedIcon} style={{ width: 20, height: 20 }} resizeMode="contain" />}
              </View>
              ...
              {blockedByMe && !isSelf ? (
                <View className="px-2 py-2">
                  ...
                  >
                    <Text className="text-sm font-medium text-white">{t('profile.unblock')}</Text>
                  </Pressable>
                </View>
              ) : (
                  ...
                  text={
                    profile.spouse != null ? (
                      <>
                        ...
                        <Text
                          style={{ color: '#33691e' }}
                          onPress={() => onOpenProfile(profile.spouse!.username, colorForName(profile.spouse!.username))}
                        >
            ...
            {followingUsers.length > 0 && (
              <View className="mb-2 bg-white pb-1" style={CARD_SHADOW}>
                ...
                >
                  <Text className="text-base" style={{ color: 'rgba(0,0,0,0.87)' }}>{t('profile.following')}</Text>
                  <Text className="text-sm" style={{ color: 'rgba(0,0,0,0.38)' }}>{t('profile.viewAll')} ›</Text>
                </Pressable>
                <View className="flex-row gap-1 px-2">
                  {followingUsers.slice(0, 5).map((friend) => (
                    ...
                    >
                      {friend.avatar != null && friend.avatar !== '' ? (
                        <Image source={{ uri: friend.avatar }} style={{ width: '100%', height: '100%' }} resizeMode="cover" />
                      ) : (
                        <View className="h-full w-full items-center justify-center" style={{ backgroundColor: colorForName(friend.username) }}>
                          <Text className="text-2xl font-medium text-white">{friend.username.charAt(0).toUpperCase()}</Text>
                        </View>
                      )}
                      <View className="absolute inset-x-0 bottom-0">
                        ...
                        <Text numberOfLines={1} className="px-1 py-1 text-center text-xs text-white">
                          @{friend.username}
                        </Text>
                      </View>
                    </Pressable>
                  ...
                  {followingUsers.length < 5 &&
                    Array.from({ length: 5 - Math.min(followingUsers.length, 5) }).map((_, index) => (
        ...
        {profile != null && !isSelf && onMessage != null && (
          <Pressable
            ...
            style={{ right: 16, bottom: 16 + insets.bottom, elevation: 4, shadowColor: '#000', shadowOpacity: 0.3, shadowRadius: 4, shadowOffset: { width: 0, height: 2 } }}
          >
