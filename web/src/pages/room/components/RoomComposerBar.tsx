...
import { compressImageForUpload, ImageTooLargeError, kulToken, toast } from '@lib';
import { useAttachPanel, useLongPress } from '@hooks';
import { SmileyGroupPanel, SmileyInput, type SmileyInputHandle } from '@components';
...
export const RoomComposerBar = forwardRef<RoomComposerHandle, RoomComposerBarProps>(
  function RoomComposerBar({ disabled, onBeforeSend, onSendText, onSendImage }, ref) {
    ...
    async function addImageFiles(files: File[]) {
      ...
      for (const file of files) {
        try {
          ...
          toast.error(
            error instanceof ImageTooLargeError ? t('chat.imageTooLarge') : t('room.sendError')
          );
        }
      }
    }
