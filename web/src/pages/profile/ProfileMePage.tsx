...
export function ProfileMePage() {
  ...
  const [spouseTarget, setSpouseTarget] = useState<{
    username: string;
    color: string;
  } | null>(null);
  ...
  const { toggleReaction, adjustCommentCount, deletePost, togglePin } =
    usePostListActions({
      posts,
      setPosts,
      reloadAfterPin,
    ...
    <>
      <FullScreenOverlay>
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="mb-2 bg-white shadow-[0_1px_2px_rgba(0,0,0,0.18)]">
            ...
            {user.bio ? (
              <p className="mt-3 px-4 text-center text-xs text-black/54">
                {user.bio}
              </p>
            ) : null}
      ...
      {spouseTarget ? (
        <UserProfileView
          ...
          onOpenFriend={(friend) =>
            setSpouseTarget({ username: friend.name, color: friend.color })
          }
        />
      ) : null}
    </>
