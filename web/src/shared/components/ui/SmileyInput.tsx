...
export const SmileyInput = forwardRef<SmileyInputHandle, SmileyInputProps>(
  function SmileyInput(
    {
      value,
      onChange,
      onEnter,
    ...
    useEffect(() => {
      function rememberSelection() {
        ...
        if (el == null || selection == null || selection.rangeCount === 0)
          return;
    ...
    function emit() {
      ...
      if (el == null) return;
      const hasContent =
        el.textContent !== '' || el.querySelector('img') != null;
    ...
    function caretRange(el: HTMLElement): Range | null {
      ...
      if (saved != null && el.contains(saved.commonAncestorContainer))
        return saved;
      return null;
    }
    ...
    function focusEditor() {
      ...
      const saved = savedRange.current;
      const range =
        saved != null && el.contains(saved.commonAncestorContainer)
          ? saved
          : rangeAtEnd(el);
    ...
    useImperativeHandle(ref, () => ({
      ...
      insertCode: (code: string, trailingSpace = false, focusAfter = true) => {
        ...
        insertNode(
          src == null ? document.createTextNode(code) : makeSmileyImg(code, src)
        );
