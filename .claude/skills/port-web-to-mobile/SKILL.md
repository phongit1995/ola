...
- **Media viewer** (xem ảnh full): web `useMediaViewerStore` → mobile có `components/MediaViewer.tsx` (host store-driven ở App root) + `MediaViewerModal` (presentational). ⚠️ Màn nào tự render trong `<Modal>` (vd `UserProfileScreen`) thì global host ở App root KHÔNG present được (iOS từ chối present Modal thứ 2 từ root khi Modal khác đang mở) → dùng state cục bộ + `<MediaViewerModal images index onClose>` render BÊN TRONG Modal của màn đó.
- **Con absolute tràn ra ngoài bounds parent** (vd avatar `-bottom-12` đè lên cover): Android không nhận touch ở phần nằm ngoài bounds parent (vẽ vẫn thấy nhưng không tap được). Fix: wrapper `paddingBottom` bằng phần tràn, đặt phần tử `absolute bottom-0` so với wrapper thay vì `-bottom-X` so với parent bị tràn.
