package com.google.android.gms.internal.ads;

import android.app.Activity;
import android.content.Context;
import android.support.annotation.VisibleForTesting;
import android.support.v4.media.session.PlaybackStateCompat;
import android.text.TextUtils;
import android.util.DisplayMetrics;
import android.util.Log;
import android.view.MotionEvent;
import android.view.View;
import java.lang.reflect.InvocationTargetException;
import java.lang.reflect.Method;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.concurrent.Callable;
import java.util.concurrent.ExecutorService;
import java.util.concurrent.TimeUnit;

/* JADX INFO: loaded from: classes.dex */
public class zzcg extends zzcf {
    private static final String TAG = "zzcg";
    private static long startTime = 0;
    private static final Object zzqr = new Object();

    @VisibleForTesting
    private static boolean zzqs = false;
    protected boolean zzqt;
    private String zzqu;
    private boolean zzqv;
    private boolean zzqw;

    @VisibleForTesting
    private zzdi zzqx;

    protected zzcg(Context context, String str, boolean z) {
        super(context);
        this.zzqt = false;
        this.zzqv = false;
        this.zzqw = false;
        this.zzqu = str;
        this.zzqt = z;
    }

    private static zzdf zza(zzcz zzczVar, MotionEvent motionEvent, DisplayMetrics displayMetrics) throws zzcw {
        Method methodZza = zzczVar.zza("ryulhTIO44CH2QVINEPOtPkpQOSdbbq7HgisLeHH33VCs+TjwYWzYttvzJcQiI1k", "dnKLchXhJgI3U7L4th0PQJWe3DOIJJ7MUji5p7t5mjc=");
        if (methodZza == null || motionEvent == null) {
            throw new zzcw();
        }
        try {
            return new zzdf((String) methodZza.invoke(null, motionEvent, displayMetrics));
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzcw(e);
        }
    }

    protected static synchronized void zza(Context context, boolean z) {
        if (!zzqs) {
            startTime = System.currentTimeMillis() / 1000;
            zzps = zzb(context, z);
            zzqs = true;
        }
    }

    private static void zza(List<Callable<Void>> list) {
        ExecutorService executorServiceZzab;
        if (zzps == null || (executorServiceZzab = zzps.zzab()) == null || list.isEmpty()) {
            return;
        }
        try {
            executorServiceZzab.invokeAll(list, ((Long) zzkb.zzik().zzd(zznk.zzbaq)).longValue(), TimeUnit.MILLISECONDS);
        } catch (InterruptedException e) {
            Log.d(TAG, String.format("class methods got exception: %s", zzdg.zza(e)));
        }
    }

    private static zzcz zzb(Context context, boolean z) {
        if (zzps == null) {
            synchronized (zzqr) {
                if (zzps == null) {
                    zzcz zzczVarZza = zzcz.zza(context, "fFhi0cTZpyVQYwMwl7BCfa0fa6esmkRUaNr4ktfJYZ8=", "", z);
                    if (zzczVarZza.isInitialized()) {
                        if (((Boolean) zzkb.zzik().zzd(zznk.zzbaw)).booleanValue()) {
                            zzczVarZza.zza("4o7tecxtkw7XaNt5hPj+0H1LvOi0SgxCIJTY9VcbazM/HSl/sFlxBFwnc8glnvoB", "RgSY6YxU2k1vLXOV3vapBnQwJDzYDlmX50wbm2tDcnw=", new Class[0]);
                        }
                        zzczVarZza.zza("1QeH3Cf7T53ayw17Ebbo9YTdhU+IFx0X5nCtC5gZQym4uicOVPXxYWmMK9k58i8n", "bHJRpFJ+2R5LAbYQUBDMyfYpLd1oiGixlpIqMJOBQPY=", Context.class);
                        zzczVarZza.zza("1DzTSPdcTVkbB0cTQEaPo/MU1E1TGypt38nYJILt/8CcQbOuYjF6hLShUHnLb5uw", "dkCo8xze5sFyp7Msrb8F1JKjVIP7LutspgZPYQvsMlo=", Context.class);
                        zzczVarZza.zza("+8WN7RfVBrTKK5NXM+y9JA4Ygu2jPZokHNFGgulhBRZ32uCnyKWWGQNTE1weoQCM", "peILYfFejTn8xmazfpiZWeK2eaYjjxEBmAZ8bUD9PWU=", Context.class);
                        zzczVarZza.zza("6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", Context.class);
                        zzczVarZza.zza("f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", Context.class);
                        zzczVarZza.zza("bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", Context.class);
                        zzczVarZza.zza("ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", Context.class);
                        zzczVarZza.zza("PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", Context.class);
                        zzczVarZza.zza("ryulhTIO44CH2QVINEPOtPkpQOSdbbq7HgisLeHH33VCs+TjwYWzYttvzJcQiI1k", "dnKLchXhJgI3U7L4th0PQJWe3DOIJJ7MUji5p7t5mjc=", MotionEvent.class, DisplayMetrics.class);
                        zzczVarZza.zza("oT49Nu6m3oNdAQwbQzvwr/RE4TwZbR8mJKypakrI/GqBgyD2niyVic8N9UaqvU10", "LS3Cw7DlWSqpRsXrajoVsCxXOedCBnQXFZcLjXzD8so=", MotionEvent.class, DisplayMetrics.class);
                        zzczVarZza.zza("1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", new Class[0]);
                        zzczVarZza.zza("R1L5/7J00Fko/kDi6VDsBhCzpp4/7wzabnzdihUS7rPLvaXLFWrERChyvYYgVJRF", "4WtEOgM4wQ7wiw1OB3pMyAbaplBizcjE6LRBT/nrDgM=", new Class[0]);
                        zzczVarZza.zza("uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", new Class[0]);
                        zzczVarZza.zza("C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", new Class[0]);
                        zzczVarZza.zza("YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", new Class[0]);
                        zzczVarZza.zza("Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", new Class[0]);
                        zzczVarZza.zza("LJD7GILw6ox/o3kYqhR5FF4FDylErrAaJ66Y8V8gtnNGF8DcetGJrYvTlHeK8kyR", "GKdCM/OywOIR4FMFS5XrDdTIpVvhoMFma/KSfRUdsbQ=", Context.class, Boolean.TYPE, Boolean.TYPE);
                        zzczVarZza.zza("nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=", StackTraceElement[].class);
                        zzczVarZza.zza("j5/WVRJsgiWNVIg0a6aXYQYocHm4rEEozf0LrIkPezct93ISwgyVBTLRIkEvztpn", "gCYklRkrI0pyKJ3j1u5Yl6Jj12a6PD1K39diuw7+f7o=", View.class, DisplayMetrics.class);
                        zzczVarZza.zza("WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", Context.class, Boolean.TYPE);
                        zzczVarZza.zza("wxabFp8V9bGKut15duzoi2M8ew0t/HWGFw3NoPp8pFanKwQpdIx0WzpFbmJaLX9T", "grSrEy+Ml8J7boJNrN9uNiXpddsqwR6A9NVesV/hHT8=", View.class, Activity.class, Boolean.TYPE);
                        zzczVarZza.zza("aH422z6bUtRL57gOABo0AU5IQDHmWvjsQgQZk64bzhjXVYUZmajAsUz/rrnMYVsw", "UAmRxgiWCEqHprf42V+e65VvIvUfFXeXbaMy5B8vzwA=", Long.TYPE);
                    }
                    zzps = zzczVarZza;
                }
            }
        }
        return zzps;
    }

    @Override // com.google.android.gms.internal.ads.zzcf
    protected final long zza(StackTraceElement[] stackTraceElementArr) throws zzcw {
        Method methodZza = zzps.zza("nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=");
        if (methodZza == null || stackTraceElementArr == null) {
            throw new zzcw();
        }
        try {
            return new zzcx((String) methodZza.invoke(null, stackTraceElementArr)).zzro.longValue();
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzcw(e);
        }
    }

    @Override // com.google.android.gms.internal.ads.zzcf
    protected final zzba zza(Context context, View view, Activity activity) {
        List arrayList;
        zzba zzbaVar = new zzba();
        if (!TextUtils.isEmpty(this.zzqu)) {
            zzbaVar.zzcw = this.zzqu;
        }
        zzcz zzczVarZzb = zzb(context, this.zzqt);
        if (zzczVarZzb.isInitialized()) {
            try {
                zzdf zzdfVarZza = zza(zzczVarZzb, this.zzpy, this.zzqq);
                zzbaVar.zzdo = zzdfVarZza.zzsp;
                zzbaVar.zzdp = zzdfVarZza.zzsq;
                zzbaVar.zzdq = zzdfVarZza.zzsr;
                if (this.zzqp) {
                    zzbaVar.zzec = zzdfVarZza.zzfr;
                    zzbaVar.zzed = zzdfVarZza.zzfp;
                }
            } catch (zzcw unused) {
            }
            zzbb zzbbVar = new zzbb();
            if (this.zzqa > 0 && zzdg.zza(this.zzqq)) {
                zzbbVar.zzfz = Long.valueOf(zzdg.zza(this.zzqh, this.zzqq));
                zzbbVar.zzga = Long.valueOf(zzdg.zza(this.zzqm - this.zzqk, this.zzqq));
                zzbbVar.zzgb = Long.valueOf(zzdg.zza(this.zzqn - this.zzql, this.zzqq));
                zzbbVar.zzge = Long.valueOf(zzdg.zza(this.zzqk, this.zzqq));
                zzbbVar.zzgf = Long.valueOf(zzdg.zza(this.zzql, this.zzqq));
                if (this.zzqp && this.zzpy != null) {
                    long jZza = zzdg.zza(((this.zzqk - this.zzqm) + this.zzpy.getRawX()) - this.zzpy.getX(), this.zzqq);
                    if (jZza != 0) {
                        zzbbVar.zzgc = Long.valueOf(jZza);
                    }
                    long jZza2 = zzdg.zza(((this.zzql - this.zzqn) + this.zzpy.getRawY()) - this.zzpy.getY(), this.zzqq);
                    if (jZza2 != 0) {
                        zzbbVar.zzgd = Long.valueOf(jZza2);
                    }
                }
            }
            try {
                zzdf zzdfVarZzb = zzb(this.zzpy);
                zzbbVar.zzdo = zzdfVarZzb.zzsp;
                zzbbVar.zzdp = zzdfVarZzb.zzsq;
                zzbbVar.zzfu = zzdfVarZzb.zzsr;
                if (this.zzqp) {
                    zzbbVar.zzfp = zzdfVarZzb.zzfp;
                    zzbbVar.zzfr = zzdfVarZzb.zzfr;
                    zzbbVar.zzft = Integer.valueOf(zzdfVarZzb.zzss.longValue() != 0 ? 1 : 0);
                    if (this.zzqb > 0) {
                        zzbbVar.zzfq = zzdg.zza(this.zzqq) ? Long.valueOf(Math.round(this.zzqg / this.zzqb)) : null;
                        zzbbVar.zzfs = Long.valueOf(Math.round(this.zzqf / this.zzqb));
                    }
                    zzbbVar.zzfw = zzdfVarZzb.zzfw;
                    zzbbVar.zzfv = zzdfVarZzb.zzfv;
                    zzbbVar.zzfx = Integer.valueOf(zzdfVarZzb.zzsv.longValue() != 0 ? 1 : 0);
                }
            } catch (zzcw unused2) {
            }
            if (this.zzqe > 0) {
                zzbbVar.zzfy = Long.valueOf(this.zzqe);
            }
            zzbaVar.zzet = zzbbVar;
            if (this.zzqa > 0) {
                zzbaVar.zzeh = Long.valueOf(this.zzqa);
            }
            if (this.zzqb > 0) {
                zzbaVar.zzeg = Long.valueOf(this.zzqb);
            }
            if (this.zzqc > 0) {
                zzbaVar.zzef = Long.valueOf(this.zzqc);
            }
            if (this.zzqd > 0) {
                zzbaVar.zzei = Long.valueOf(this.zzqd);
            }
            try {
                int size = this.zzpz.size() - 1;
                if (size > 0) {
                    zzbaVar.zzeu = new zzbb[size];
                    for (int i = 0; i < size; i++) {
                        zzdf zzdfVarZza2 = zza(zzps, this.zzpz.get(i), this.zzqq);
                        zzbb zzbbVar2 = new zzbb();
                        zzbbVar2.zzdo = zzdfVarZza2.zzsp;
                        zzbbVar2.zzdp = zzdfVarZza2.zzsq;
                        zzbaVar.zzeu[i] = zzbbVar2;
                    }
                }
            } catch (zzcw unused3) {
                zzbaVar.zzeu = null;
            }
            arrayList = new ArrayList();
            if (zzczVarZzb.zzab() != null) {
                int iZzx = zzczVarZzb.zzx();
                arrayList.add(new zzdt(zzczVarZzb, zzbaVar));
                arrayList.add(new zzdw(zzczVarZzb, "uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", zzbaVar, iZzx, 1));
                arrayList.add(new zzdr(zzczVarZzb, "1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", zzbaVar, startTime, iZzx, 25));
                arrayList.add(new zzdq(zzczVarZzb, "C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", zzbaVar, iZzx, 44));
                arrayList.add(new zzdv(zzczVarZzb, "f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", zzbaVar, iZzx, 12));
                arrayList.add(new zzdx(zzczVarZzb, "bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", zzbaVar, iZzx, 3));
                arrayList.add(new zzdu(zzczVarZzb, "YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", zzbaVar, iZzx, 22));
                arrayList.add(new zzdp(zzczVarZzb, "6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", zzbaVar, iZzx, 5));
                arrayList.add(new zzee(zzczVarZzb, "ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", zzbaVar, iZzx, 48));
                arrayList.add(new zzdm(zzczVarZzb, "PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", zzbaVar, iZzx, 49));
                arrayList.add(new zzeb(zzczVarZzb, "Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", zzbaVar, iZzx, 51));
                arrayList.add(new zzea(zzczVarZzb, "nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=", zzbaVar, iZzx, 45, new Throwable().getStackTrace()));
                arrayList.add(new zzef(zzczVarZzb, "j5/WVRJsgiWNVIg0a6aXYQYocHm4rEEozf0LrIkPezct93ISwgyVBTLRIkEvztpn", "gCYklRkrI0pyKJ3j1u5Yl6Jj12a6PD1K39diuw7+f7o=", zzbaVar, iZzx, 57, view));
                arrayList.add(new zzdz(zzczVarZzb, "WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", zzbaVar, iZzx, 61));
                if (((Boolean) zzkb.zzik().zzd(zznk.zzbar)).booleanValue()) {
                    arrayList.add(new zzdl(zzczVarZzb, "wxabFp8V9bGKut15duzoi2M8ew0t/HWGFw3NoPp8pFanKwQpdIx0WzpFbmJaLX9T", "grSrEy+Ml8J7boJNrN9uNiXpddsqwR6A9NVesV/hHT8=", zzbaVar, iZzx, 62, view, activity));
                }
                if (((Boolean) zzkb.zzik().zzd(zznk.zzbat)).booleanValue()) {
                    arrayList.add(new zzec(zzczVarZzb, "aH422z6bUtRL57gOABo0AU5IQDHmWvjsQgQZk64bzhjXVYUZmajAsUz/rrnMYVsw", "UAmRxgiWCEqHprf42V+e65VvIvUfFXeXbaMy5B8vzwA=", zzbaVar, iZzx, 53, this.zzqx));
                }
            }
        } else {
            zzbaVar.zzdu = Long.valueOf(PlaybackStateCompat.ACTION_PREPARE);
            arrayList = Arrays.asList(new zzdt(zzczVarZzb, zzbaVar));
        }
        zza((List<Callable<Void>>) arrayList);
        return zzbaVar;
    }

    @Override // com.google.android.gms.internal.ads.zzcf
    protected final zzba zza(Context context, zzax zzaxVar) {
        zzba zzbaVar = new zzba();
        if (!TextUtils.isEmpty(this.zzqu)) {
            zzbaVar.zzcw = this.zzqu;
        }
        zzcz zzczVarZzb = zzb(context, this.zzqt);
        if (zzczVarZzb.zzab() != null) {
            zza(zza(zzczVarZzb, zzbaVar, (zzax) null));
        }
        return zzbaVar;
    }

    protected List<Callable<Void>> zza(zzcz zzczVar, zzba zzbaVar, zzax zzaxVar) {
        int iZzx = zzczVar.zzx();
        ArrayList arrayList = new ArrayList();
        if (!zzczVar.isInitialized()) {
            zzbaVar.zzdu = Long.valueOf(PlaybackStateCompat.ACTION_PREPARE);
            return arrayList;
        }
        arrayList.add(new zzdo(zzczVar, "LJD7GILw6ox/o3kYqhR5FF4FDylErrAaJ66Y8V8gtnNGF8DcetGJrYvTlHeK8kyR", "GKdCM/OywOIR4FMFS5XrDdTIpVvhoMFma/KSfRUdsbQ=", zzbaVar, iZzx, 27, zzaxVar));
        arrayList.add(new zzdr(zzczVar, "1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", zzbaVar, startTime, iZzx, 25));
        arrayList.add(new zzdw(zzczVar, "uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", zzbaVar, iZzx, 1));
        arrayList.add(new zzdy(zzczVar, "+8WN7RfVBrTKK5NXM+y9JA4Ygu2jPZokHNFGgulhBRZ32uCnyKWWGQNTE1weoQCM", "peILYfFejTn8xmazfpiZWeK2eaYjjxEBmAZ8bUD9PWU=", zzbaVar, iZzx, 31));
        arrayList.add(new zzed(zzczVar, "R1L5/7J00Fko/kDi6VDsBhCzpp4/7wzabnzdihUS7rPLvaXLFWrERChyvYYgVJRF", "4WtEOgM4wQ7wiw1OB3pMyAbaplBizcjE6LRBT/nrDgM=", zzbaVar, iZzx, 33));
        arrayList.add(new zzdn(zzczVar, "1DzTSPdcTVkbB0cTQEaPo/MU1E1TGypt38nYJILt/8CcQbOuYjF6hLShUHnLb5uw", "dkCo8xze5sFyp7Msrb8F1JKjVIP7LutspgZPYQvsMlo=", zzbaVar, iZzx, 29));
        arrayList.add(new zzdp(zzczVar, "6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", zzbaVar, iZzx, 5));
        arrayList.add(new zzdv(zzczVar, "f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", zzbaVar, iZzx, 12));
        arrayList.add(new zzdx(zzczVar, "bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", zzbaVar, iZzx, 3));
        arrayList.add(new zzdq(zzczVar, "C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", zzbaVar, iZzx, 44));
        arrayList.add(new zzdu(zzczVar, "YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", zzbaVar, iZzx, 22));
        arrayList.add(new zzee(zzczVar, "ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", zzbaVar, iZzx, 48));
        arrayList.add(new zzdm(zzczVar, "PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", zzbaVar, iZzx, 49));
        arrayList.add(new zzeb(zzczVar, "Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", zzbaVar, iZzx, 51));
        arrayList.add(new zzdz(zzczVar, "WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", zzbaVar, iZzx, 61));
        return arrayList;
    }

    @Override // com.google.android.gms.internal.ads.zzcf
    protected final zzdf zzb(MotionEvent motionEvent) throws zzcw {
        Method methodZza = zzps.zza("oT49Nu6m3oNdAQwbQzvwr/RE4TwZbR8mJKypakrI/GqBgyD2niyVic8N9UaqvU10", "LS3Cw7DlWSqpRsXrajoVsCxXOedCBnQXFZcLjXzD8so=");
        if (methodZza == null || motionEvent == null) {
            throw new zzcw();
        }
        try {
            return new zzdf((String) methodZza.invoke(null, motionEvent, this.zzqq));
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzcw(e);
        }
    }

    @Override // com.google.android.gms.internal.ads.zzcf, com.google.android.gms.internal.ads.zzce
    public final void zzb(View view) {
        if (((Boolean) zzkb.zzik().zzd(zznk.zzbat)).booleanValue()) {
            if (this.zzqx == null) {
                this.zzqx = new zzdi(zzps, view);
            } else {
                this.zzqx.zzd(view);
            }
        }
    }
}
