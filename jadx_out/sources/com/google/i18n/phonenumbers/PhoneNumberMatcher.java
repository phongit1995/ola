...
final class PhoneNumberMatcher implements Iterator<PhoneNumberMatch> {
    ...
    static {
        ...
        PATTERN = Pattern.compile("(?:" + str4 + str2 + ")" + strLimit + str3 + "(?:" + str2 + str3 + ")" + strLimit3 + "(?:" + PhoneNumberUtil.EXTN_PATTERNS_FOR_MATCHING + ")?", 66);
    }
    ...
    static boolean checkNumberGroupingIsValid(Phonenumber.PhoneNumber phoneNumber, CharSequence charSequence, PhoneNumberUtil phoneNumberUtil, NumberGroupingChecker numberGroupingChecker) {
        ...
        Iterator<Phonemetadata.NumberFormat> it2 = alternateFormatsForCountry.numberFormats().iterator();
        while (it2.hasNext()) {
            if (numberGroupingChecker.checkGroups(phoneNumberUtil, phoneNumber, sbNormalizeDigits, getNationalNumberGroups(phoneNumberUtil, phoneNumber, it2.next()))) {
                return true;
            }
        }
        return false;
    ...
    private PhoneNumberMatch extractMatch(CharSequence charSequence, int i) {
        ...
        return andVerify != null ? andVerify : extractInnerMatch(charSequence, i);
    }
    ...
    private static String[] getNationalNumberGroups(PhoneNumberUtil phoneNumberUtil, Phonenumber.PhoneNumber phoneNumber, Phonemetadata.NumberFormat numberFormat) {
        ...
        if (numberFormat == null) {
            ...
            nsnUsingPattern = str.substring(str.indexOf(45) + 1, iIndexOf);
        } else {
            nsnUsingPattern = phoneNumberUtil.formatNsnUsingPattern(phoneNumberUtil.getNationalSignificantNumber(phoneNumber), numberFormat, PhoneNumberUtil.PhoneNumberFormat.RFC3966);
        }
        return nsnUsingPattern.split("-");
    }
