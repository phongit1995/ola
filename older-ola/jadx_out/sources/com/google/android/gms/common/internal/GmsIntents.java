...
public class GmsIntents {
    ...
    public static void sendIcingContactChangedBroadcast(Context context, boolean z) {
        Intent intentPutExtra = new Intent(ACTION_ICING_CONTACT_CHANGED).setPackage("com.google.android.gms").putExtra(EXTRA_ICING_CONTACT_CHANGED_IS_SIGNIFICANT, z);
        if (Log.isLoggable("GmsIntents", 2)) {
            String action = intentPutExtra.getAction();
    ...
    private static void zza(String str, Context context, String str2, int[] iArr) {
        Intent intentPutExtra = new Intent(ACTION_UDC_SETTING_CHANGED).setPackage(str).putExtra(EXTRA_UDC_ACCOUNT_NAME, str2).putExtra(EXTRA_UDC_SETTING_ID_LIST, iArr);
