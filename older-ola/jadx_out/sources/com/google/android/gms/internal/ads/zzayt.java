...
public final class zzayt {
    ...
    public static KeyPair zza(ECParameterSpec eCParameterSpec) throws GeneralSecurityException {
        KeyPairGenerator keyPairGeneratorZzek = zzayy.zzdoe.zzek("EC");
        keyPairGeneratorZzek.initialize(eCParameterSpec);
        return keyPairGeneratorZzek.generateKeyPair();
    }

    public static ECPublicKey zza(zzayv zzayvVar, byte[] bArr, byte[] bArr2) throws GeneralSecurityException {
        ECParameterSpec eCParameterSpecZza = zza(zzayvVar);
        ECPoint eCPoint = new ECPoint(new BigInteger(1, bArr), new BigInteger(1, bArr2));
        zza(eCPoint, eCParameterSpecZza.getCurve());
        return (ECPublicKey) zzayy.zzdof.zzek("EC").generatePublic(new ECPublicKeySpec(eCPoint, eCParameterSpecZza));
    }
    ...
    public static ECParameterSpec zza(zzayv zzayvVar) throws NoSuchAlgorithmException {
        ...
        switch (zzayvVar) {
            ...
            case NIST_P521:
                ...
                break;
            default:
                String strValueOf = String.valueOf(zzayvVar);
    ...
    static void zza(ECPoint eCPoint, EllipticCurve ellipticCurve) throws GeneralSecurityException {
        ...
        if (affineX.signum() == -1 || affineX.compareTo(bigIntegerZza) != -1) {
            throw new GeneralSecurityException("x is out of range");
        }
        if (affineY.signum() == -1 || affineY.compareTo(bigIntegerZza) != -1) {
    ...
    public static byte[] zza(ECPrivateKey eCPrivateKey, ECPoint eCPoint) throws GeneralSecurityException {
        ...
        KeyAgreement keyAgreementZzek = zzayy.zzdod.zzek("ECDH");
        keyAgreementZzek.init(eCPrivateKey);
        keyAgreementZzek.doPhase(publicKeyGeneratePublic, true);
        byte[] bArrGenerateSecret = keyAgreementZzek.generateSecret();
