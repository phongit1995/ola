...
public class MessengerUtils {
    ...
    public static void finishShareToMessenger(Activity activity, ShareToMessengerParams shareToMessengerParams) {
        ...
        if (categories.contains(ORCA_THREAD_CATEGORY_20150314)) {
            Bundle bundleB = c.b(intent);
            Intent intent2 = new Intent();
            if (!categories.contains(ORCA_THREAD_CATEGORY_20150314)) {
                throw new RuntimeException();
            }
            intent2.putExtra(EXTRA_PROTOCOL_VERSION, PROTOCOL_VERSION_20150314);
            intent2.putExtra(EXTRA_THREAD_TOKEN_KEY, bundleB.getString(EXTRA_THREAD_TOKEN_KEY));
    ...
    public static MessengerThreadParams getMessengerThreadParamsForIntent(Intent intent) {
        ...
        Bundle bundleB = c.b(intent);
        String string = bundleB.getString(EXTRA_THREAD_TOKEN_KEY);
        String string2 = bundleB.getString(EXTRA_METADATA);
        String string3 = bundleB.getString(EXTRA_PARTICIPANTS);
        boolean z = bundleB.getBoolean(EXTRA_IS_REPLY);
        boolean z2 = bundleB.getBoolean(EXTRA_IS_COMPOSE);
